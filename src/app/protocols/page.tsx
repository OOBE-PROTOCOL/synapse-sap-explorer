...
export default function ProtocolsPage() {
  ...
  const protocols = useMemo(() => {
    ...
    const addProtocol = (raw: string | null | undefined) => {
      ...
      if (!protocolDisplayByNorm.has(norm))
        protocolDisplayByNorm.set(norm, display);
    };
    ...
    for (const a of agentsData?.agents ?? []) {
      ...
      for (const cap of a.identity?.capabilities ?? [])
        addProtocol(cap.protocolId);
    }

    const list = Array.from(protocolDisplayByNorm.entries()).map(
      ([normId, displayId]) => {
        const agents = (agentsData?.agents ?? [])
          .filter((agent: SerializedDiscoveredAgent) => {
            const id = agent.identity;
            if (!id) return false;
            const inProtocols = (id.protocols ?? []).some(
              (proto) => normalizeProtocolId(proto) === normId,
            );
            const inCapabilities = (id.capabilities ?? []).some(
              (cap) => normalizeProtocolId(cap.protocolId) === normId,
            );
        ...
        const topAgent =
          [...agents].sort((a, b) => b.totalCalls - a.totalCalls)[0] ?? null;
    ...
    return list.sort(
      (a, b) => b.agentCount - a.agentCount || a.id.localeCompare(b.id),
    );
  ...
  const stats = useMemo(() => {
    ...
    const totalCapabilities = protocols.reduce(
      (sum, p) => sum + p.capabilities.length,
      0,
    );
    return {
      total,
  ...
  const filtered = useMemo(() => {
    ...
    const list = protocols.filter((p) => {
      if (agentFilter !== "all" && !p.agents.some((a) => a.pda === agentFilter))
        return false;

      if (activityFilter === "high" && p.agentCount < 5) return false;
      if (
        activityFilter === "medium" &&
        (p.agentCount < 2 || p.agentCount >= 5)
      )
        return false;
      if (activityFilter === "low" && p.agentCount >= 2) return false;

      if (capabilityFilter === "with" && p.capabilities.length === 0)
        return false;
      if (capabilityFilter === "without" && p.capabilities.length > 0)
        return false;
      ...
      ? [
          {
            ...
            label: "Agent",
            value:
              filterAgents.find(([pda]) => pda === agentFilter)?.[1] ??
              agentFilter,
      ...
      ? [
          {
            ...
            onClear: () => setCapabilityFilter("all"),
          },
        ]
      : []),
  ];
        ...
        <Select value={agentFilter} onValueChange={setAgentFilter}>
          ...
          <SelectContent>
            ...
            {filterAgents.map(([pda, name]) => (
              <SelectItem key={pda} value={pda} className="text-xs">
                {name}
              </SelectItem>
            ))}
          </SelectContent>
        ...
        <Card className="bg-neutral-900 border-red-900/40">
          <CardContent className="py-8 flex flex-col items-center gap-3 text-center">
            ...
            <p className="text-sm text-neutral-200">
              Unable to load protocols right now.
            </p>
            <p className="text-xs text-neutral-500">
              Try again to refresh graph and agent metadata from DB.
            </p>
            <Button
              size="sm"
              variant="outline"
      ...
      <CardContent className="px-5 pt-3 pb-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="rounded-lg bg-neutral-800/50 px-3 py-2">
            <p className="text-lg font-bold tabular-nums text-white font-mono">
              {protocol.agentCount}
            </p>
          ...
          <div className="rounded-lg border border-neutral-800 bg-neutral-800/30 px-3 py-2.5">
            ...
            >
              ...
              <span className="truncate">
                {protocol.topAgent.name ??
                  shortAddress(
                    protocol.topAgent.wallet ?? protocol.topAgent.pda,
                  )}
              </span>
            </Link>
          </div>
              ...
              {visibleAgents.map((agent) => (
                ...
                >
                  ...
                  <span className="truncate max-w-[170px]">
                    {agent.name ?? shortAddress(agent.wallet ?? agent.pda)}
                  </span>
                </Link>
              ))}
        ...
        {protocol.capabilities.length > 0 && (
          <div className="border-t border-neutral-800 pt-3">
            ...
            <div className="flex flex-wrap gap-1">
              ...
              {protocol.capabilities.length > 6 && (
