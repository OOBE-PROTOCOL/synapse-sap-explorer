...
export default function VaultsPage() {
  ...
  const stats = useMemo(() => {
    ...
    const totalInscriptions = vaults.reduce((s, v) => s + Number(v.totalInscriptions), 0);
    const totalBytes = vaults.reduce((s, v) => s + Number(v.totalBytesInscribed), 0);
  ...
  return (
    <ExplorerPageShell
      ...
      badge={<Badge variant="outline" className="text-xs tabular-nums">{data?.total ?? 0} vaults</Badge>}
      actions={
      ...
      stats={!loading ? (
        <>
          <ExplorerMetric icon={<Vault className="h-3.5 w-3.5" />} label="Total Vaults" value={stats.total} accent="primary" />
          <ExplorerMetric icon={<Clock className="h-3.5 w-3.5" />} label="Total Sessions" value={stats.totalSessions} accent="emerald" />
          <ExplorerMetric icon={<FileText className="h-3.5 w-3.5" />} label="Inscriptions" value={stats.totalInscriptions.toLocaleString()} accent="cyan" />
          <ExplorerMetric icon={<Database className="h-3.5 w-3.5" />} label="Data Inscribed" value={formatBytes(stats.totalBytes)} accent="amber" />
        </>
      ) : undefined}
    >
      {loading ? (
        <Card><CardContent className="p-4 space-y-3">{Array.from({ length: 6 }).map((_, i) => <Skeleton key={i} className="h-14 w-full" />)}</CardContent></Card>
      ) : error ? (
        <Card><CardContent className="py-8 text-center"><p className="text-sm text-destructive">{error}</p></CardContent></Card>
      ) : filtered.length === 0 ? (
        <EmptyState message={search ? 'No vaults match your search' : 'No vaults found on-chain'} />
      ) : (
        <Card>
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-right hidden sm:table-cell">Inscriptions</TableHead>
                <TableHead className="text-right hidden lg:table-cell">Data Size</TableHead>
                <TableHead className="text-right hidden lg:table-cell">Memory Layers</TableHead>
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell className="text-right hidden sm:table-cell">
                      <span className="text-sm tabular-nums">{Number(vault.totalInscriptions).toLocaleString()}</span>
                    </TableCell>
                    <TableCell className="text-right hidden lg:table-cell">
                      <span className="text-xs tabular-nums text-muted-foreground">{formatBytes(Number(vault.totalBytesInscribed))}</span>
                    </TableCell>
                    ...
                    <TableCell className="hidden md:table-cell">
                      <span className="text-xs text-muted-foreground">
                        {vault.createdAt ? new Date(Number(vault.createdAt) * (Number(vault.createdAt) > 1e12 ? 1 : 1000)).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '—'}
                      </span>
                    </TableCell>
                  </TableRow>
                );
