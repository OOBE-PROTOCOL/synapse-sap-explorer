...
export default function ToolsPage() {
  ...
  const methods = useMemo(() => {
    ...
    enrichedTools.forEach((t) => {
      ...
      const m = typeof t.descriptor.httpMethod === 'object'
  ...
  const stats = useMemo(() => {
    ...
    const totalCallsSettled = enrichedTools.reduce((s, t) => s + t.callsSettled, 0);
    const compound = enrichedTools.filter((t) => t.descriptor?.isCompound).length;
  ...
  const filtered = useMemo(() => {
    const list = enrichedTools.filter((t) => {
      ...
      /* Method */
      if (methodFilter !== 'all') {
        const m = typeof d.httpMethod === 'object' ? Object.keys(d.httpMethod)[0] : String(d.httpMethod);
    ...
    list.sort((a, b) => {
      ...
      let cmp = 0;
      switch (sortKey) {
        case 'name': cmp = da.toolName.localeCompare(db.toolName); break;
        case 'calls': cmp = (a.callsSettled ?? 0) - (b.callsSettled ?? 0); break;
        case 'params': cmp = da.paramsCount - db.paramsCount; break;
        case 'created': cmp = Number(da.createdAt) - Number(db.createdAt); break;
        case 'updated': cmp = Number(da.updatedAt) - Number(db.updatedAt); break;
      }
      return sortDir === 'desc' ? -cmp : cmp;
    });

    return list;
  }, [enrichedTools, categoryFilter, methodFilter, statusFilter, search, sortKey, sortDir]);

  const { page, perPage, setPage, setPerPage, paginate } = usePagination(filtered.length, 25);
  const paginatedTools = useMemo(() => paginate(filtered), [paginate, filtered]);
  ...
  return (
    <ExplorerPageShell
      ...
      badge={<Badge variant="outline" className="text-xs tabular-nums">{data?.total ?? 0} tools</Badge>}
      actions={
      ...
      stats={!loading ? (
        <>
          <ExplorerMetric icon={<Wrench className="h-3.5 w-3.5" />} label="Total Tools" value={stats.total} accent="primary" />
          <ExplorerMetric icon={<Activity className="h-3.5 w-3.5" />} label="Active" value={stats.active} sub={stats.total > 0 ? `${Math.round((stats.active / stats.total) * 100)}%` : undefined} accent="emerald" />
              ...
              <div className="space-y-2">
                ...
                {categories.length > 0 && (
                  <Select value={categoryFilter} onValueChange={setCategoryFilter}>
                    <SelectTrigger className="h-7 w-full text-xs"><SelectValue placeholder="All categories" /></SelectTrigger>
                    <SelectContent>
                ...
                {methods.length > 0 && (
                  <Select value={methodFilter} onValueChange={setMethodFilter}>
                    <SelectTrigger className="h-7 w-full text-xs"><SelectValue placeholder="All methods" /></SelectTrigger>
                    <SelectContent>
                ...
                <Select value={statusFilter} onValueChange={setStatusFilter}>
                  <SelectTrigger className="h-7 w-full text-xs"><SelectValue placeholder="All status" /></SelectTrigger>
                  <SelectContent>
    ...
    >
      ...
      {!loading && (
        <div className="flex items-center">
          <p className="text-xs text-muted-foreground">
            Showing <span className="font-medium text-foreground">{filtered.length}</span> of {enrichedTools.length} tools
          </p>
        </div>
      )}
      ...
      {/* Table */}
      {loading ? (
        <Card><CardContent className="p-4 space-y-3">{Array.from({ length: 8 }).map((_, i) => (<Skeleton key={i} className="h-14 w-full" />))}</CardContent></Card>
      ) : error ? (
        <Card><CardContent className="py-8 text-center"><p className="text-sm text-destructive">{error}</p></CardContent></Card>
      ) : filtered.length === 0 ? (
        <EmptyState message={search || categoryFilter !== 'all' || methodFilter !== 'all' || statusFilter !== 'all' ? 'No tools match your filters' : 'No tools discovered on-chain'} />
      ) : (
        <Card>
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead className="cursor-pointer select-none" onClick={() => toggleSort('name')}>
                  <span className="inline-flex items-center">Tool <SortIcon col="name" /></span>
                </TableHead>
                <TableHead className="hidden sm:table-cell">Method</TableHead>
                <TableHead className="hidden sm:table-cell">Category</TableHead>
                <TableHead className="hidden md:table-cell">
                  <span className="inline-flex items-center"><User className="h-3 w-3 mr-1" /> Creator</span>
                </TableHead>
                <TableHead className="hidden lg:table-cell">Schema</TableHead>
                <TableHead className="text-right cursor-pointer select-none" onClick={() => toggleSort('calls')}>
                  <span className="inline-flex items-center justify-end">Calls <SortIcon col="calls" /></span>
                </TableHead>
                <TableHead className="hidden lg:table-cell text-right cursor-pointer select-none" onClick={() => toggleSort('params')}>
                  <span className="inline-flex items-center justify-end">Params <SortIcon col="params" /></span>
                </TableHead>
            ...
            <TableBody>
              {paginatedTools.map((tool) => {
                ...
                const hasInputSchema = d.inputSchemaHash?.some((b: number) => b !== 0);
                const hasOutputSchema = d.outputSchemaHash?.some((b: number) => b !== 0);
                const hasInscribedSchema = Boolean(tool.hasInscribedSchema);
                return (
                  <TableRow key={tool.pda} className="cursor-pointer group hover:bg-muted/30 transition-colors" onClick={() => router.push(`/tools/${tool.pda}`)}>
                      <TableCell>
                      ...
                      <TableCell className="hidden md:table-cell">
                        <AgentTag address={tool.agentWallet ?? d.agent} className="text-xs" />
                      </TableCell>
                      ...
                      <TableCell className="text-right">
                        <span className="text-sm font-semibold tabular-nums">{tool.callsSettled.toLocaleString()}</span>
                      </TableCell>
                      <TableCell className="hidden lg:table-cell text-right">
                        <span className="text-xs tabular-nums text-muted-foreground">
                          <span className="text-foreground font-medium">{d.requiredParams}</span>/{d.paramsCount}
                        </span>
                      </TableCell>
