...
const PROGRAM_META: Record<string, ProgramMeta> = {
  [SAP_ADDRESS]: { short: "SAP", type: "sap", color: "" },
...
function ActionLabel({ tx }: { tx: SapTx }) {
  ...
  if (names.length > 0)
    return <span className="text-xs text-muted-foreground">{names[0]}</span>;
...
function resolveCounterparties(
  tx: SapTx,
  agentMap: AgentMap,
  escrowMap: EscrowMap,
): Party[] {
    ...
    >
      ...
      <TableCell>
        <div className="flex items-center gap-2">
          <span
            className="font-mono text-sm text-primary group-hover:underline"
            title={tx.signature}
          >
      ...
      <TableCell className="hidden lg:table-cell">
        {counterparties.length === 1 ? (
          ...
          <AddrLabel
            address={counterparties[0].address}
            label={counterparties[0].label}
            link={counterparties[0].link}
          />
        ) : counterparties.length > 1 ? (
            ...
            <span className="font-mono text-xs tabular-nums text-foreground">
              {tx.value.amount < 0.01
                ...
                : tx.value.amount < 1000
                  ...
                  : tx.value.amount.toLocaleString("en-US", {
                      maximumFractionDigits: 2,
                    })}
            </span>
            <span
              className={`text-[10px] font-medium ${
                tx.value.symbol === "USDC"
                  ? "text-emerald-400"
                  : tx.value.symbol === "SOL"
                    ? "text-primary"
...
export default function TransactionsPage() {
  ...
  const fetchAgentMap = useCallback(async (signal?: AbortSignal) => {
    try {
      ...
      if (
        ...
        console.warn("[txs] agent map fetch failed:", (e as Error).message);
    }
  }, []);
  ...
  const fetchEscrowMap = useCallback(async (signal?: AbortSignal) => {
    try {
      ...
      if (
        ...
        console.warn("[txs] escrow map fetch failed:", (e as Error).message);
    }
  }, []);
  ...
  if (hideSpam) {
    filtered = filtered.filter(
      (tx) =>
        ...
        Math.abs(tx.signerBalanceChange) > 10000,
    );
  }
  ...
  const stats = useMemo(() => {
    ...
    const oldest = times.length
      ? new Date(Math.min(...times) * 1000).toLocaleDateString(undefined, {
          month: "short",
          day: "numeric",
        })
      : "—";
    const newest = times.length
      ? new Date(Math.max(...times) * 1000).toLocaleDateString(undefined, {
          month: "short",
          day: "numeric",
        })
      : "—";
    return {
      success,
      successRate: txs.length ? Math.round((success / txs.length) * 100) : 0,
  ...
  const pageRange = useMemo(() => {
    const pageTimes = displayed
      .map((t) => t.blockTime)
      .filter(Boolean) as number[];
    if (pageTimes.length === 0) return { oldest: "—", newest: "—" };
    return {
      oldest: new Date(Math.min(...pageTimes) * 1000).toLocaleDateString(
        undefined,
        { month: "short", day: "numeric" },
      ),
      newest: new Date(Math.max(...pageTimes) * 1000).toLocaleDateString(
        undefined,
        { month: "short", day: "numeric" },
      ),
    };
      ...
      stats={
        <>
          ...
          <ExplorerMetric
            ...
            sub={`${stats.success} / ${txs.length}`}
            accent="emerald"
          />
          <ExplorerMetric
            label="Page Range"
            value={`${pageRange.oldest} – ${pageRange.newest}`}
            icon={<Calendar className="h-4 w-4" />}
            sub={`page ${safePage} · ${start}-${end}`}
            accent="amber"
          />
            ...
            <div className="p-4 w-full">
              <div className="grid grid-cols-2 gap-2 w-full">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  >
                    <span
                      className={cn(
                        ...
                        hideSpam ? "bg-primary" : "bg-neutral-600",
                      )}
                    />
                    Hide Spam
                  </button>
                  <button
                    onClick={() => {
                      setHideFailed(!hideFailed);
                      setPage(1);
                    }}
                    className={cn(
                      "inline-flex items-center gap-1.5 px-1.5 h-7 w-full text-xs rounded-md border transition-colors whitespace-nowrap",
                      hideFailed
                        ? "bg-primary/10 border-primary/30 text-primary"
                        : "bg-transparent border-neutral-700 text-neutral-400 hover:border-neutral-500 hover:text-white",
                    )}
                  >
                    <span
                      className={cn(
                        "h-1.5 w-1.5 rounded-full shrink-0",
                        hideFailed ? "bg-primary" : "bg-neutral-600",
                      )}
                    />
                    Hide Failed
                  </button>
