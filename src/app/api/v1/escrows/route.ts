...
import { listPublicEscrows } from '~/lib/api/public/escrows';
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const limit = optionalPositiveInt(req.nextUrl.searchParams.get('limit'), 'limit', 500) ?? 100;
    const result = await listPublicEscrows({ limit });

    return ok(result.escrows, {
      requestId,
