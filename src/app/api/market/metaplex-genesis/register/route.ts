...
import { registerGenesisLaunch } from '~/lib/metaplex/genesis';
import type {
...
function validatePayload(payload: MetaplexGenesisRegisterLaunchRequest): string | null {
  if (!payload.genesisAccount || !isValidPubkey(payload.genesisAccount)) {
    return 'Invalid genesisAccount public key';
  }
  ...
  if (payload.network && l.network && payload.network !== l.network) {
    return 'network mismatch between top-level network and launch.network';
  }
...
export async function POST(request: Request) {
  const body = (await request.json().catch(() => null)) as MetaplexGenesisRegisterLaunchRequest | null;
  if (!body) {
  ...
  const result = await registerGenesisLaunch(body);
  if (result.error || !result.data) {
    return NextResponse.json(
      {
        success: false,
        error: result.error ?? 'Register launch failed',
      },
